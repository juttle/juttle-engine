...
function create_job_from_bundle(req, res, next) {
...
function create_job(req, res, next) {
    if (_.has(req.body, 'path')) {
        create_job_from_path(req, res, next);
    } else {
        create_job_from_bundle(req, res, next);
    }
}
